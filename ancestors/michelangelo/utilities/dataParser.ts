export default (data: any, type : number) => {
  ...
  if (type === 1) {
    ...
    parsedData.release_dates = data.release_dates.find((releaseDate: any) => {
      if (releaseDate.region == 2 || 8) return releaseDate;
    });
    if (parsedData.release_dates) {
    ...
    if (parsedData.involved_companies) {
      const developers =
